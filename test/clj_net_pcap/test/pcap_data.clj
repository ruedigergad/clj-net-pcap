...
(test/deftest test-guess-class-c-net
  (let [ip-addr "192.168.42.123"]
    (test/is (= :class-c (pcap-data/network-class ip-addr)))
    (test/is (= "192.168.42.0" (pcap-data/guess-subnet ip-addr)))
    (test/is (= "255.255.255.0" (pcap-data/guess-subnet-mask ip-addr)))
    (test/is (= 24 (pcap-data/guess-subnet-mask-bits ip-addr)))))

(test/deftest test-guess-class-a-net
  (let [ip-addr "10.123.45.67"]
    (test/is (= :class-a (pcap-data/network-class ip-addr)))
    (test/is (= "10.0.0.0" (pcap-data/guess-subnet ip-addr)))
    (test/is (= "255.0.0.0" (pcap-data/guess-subnet-mask ip-addr)))
    (test/is (= 8 (pcap-data/guess-subnet-mask-bits ip-addr)))))

(test/deftest test-guess-class-b-net
  (let [ip-addr "172.16.123.45"]
    (test/is (= :class-b (pcap-data/network-class ip-addr)))
    (test/is (= "172.16.0.0" (pcap-data/guess-subnet ip-addr)))
    (test/is (= "255.255.0.0" (pcap-data/guess-subnet-mask ip-addr)))
    (test/is (= 16 (pcap-data/guess-subnet-mask-bits ip-addr)))))

(test/deftest test-guess-class-b-net-2
  (let [ip-addr "172.31.123.45"]
    (test/is (= :class-b (pcap-data/network-class ip-addr)))
    (test/is (= "172.31.0.0" (pcap-data/guess-subnet ip-addr)))
    (test/is (= "255.255.0.0" (pcap-data/guess-subnet-mask ip-addr)))
    (test/is (= 16 (pcap-data/guess-subnet-mask-bits ip-addr)))))
