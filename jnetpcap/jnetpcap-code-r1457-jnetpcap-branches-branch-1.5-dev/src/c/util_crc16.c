 ...
 *
 * $Id: crc16.c 20133 2006-12-14 00:16:06Z sfisher $
 *
...
static const uint32_t crc16_ccitt_table_reverse[256] =
{
 ...
 */
static uint16_t crc16_unreflected(const uint8_t *buf, uint32_t len,
				 uint16_t crc_in, const uint32_t table[])
{
...
static uint16_t crc16_reflected(const uint8_t *buf, uint32_t len,
                                uint16_t crc_in, const uint32_t table[])
{
    /* we use uint32_ts, rather than uint16_ts, as they are likely to be
       faster. We just ignore the top 16 bits and let them do what they want.
...
{
    return crc16_unreflected(buf,len,crc16_ccitt_start,crc16_ccitt_table_reverse)
       ^ crc16_ccitt_xorout;
}

uint16_t crc16_x25_ccitt(const uint8_t *buf, uint32_t len)
{
    return crc16_unreflected(buf,len,crc16_ccitt_start,crc16_ccitt_table);
}

uint16_t crc16_ccitt_seed(const uint8_t *buf, uint32_t len, uint16_t seed)
{
    return crc16_unreflected(buf,len,seed,crc16_ccitt_table_reverse)
       ^ crc16_ccitt_xorout;
}
